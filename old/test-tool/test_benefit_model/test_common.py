...
def set_premise_a(neo_path_a, neo_path_b):
    ...
    # 用户A创建角色A
    (result, response) = bind_role_function(contract_address_B, Common.ontID_A , Common.roleA_hex, ["contractB_Func_A"])
    if not result:
        raise(Error("bind_role_function error [1]"))

    # setp 1 用户A绑定角色A
    (result, response) = bind_user_role(contract_address_B, Common.ontID_A, Common.roleA_hex, [Common.ontID_A])
    if not result:
