...
class TestConsensus(ParametrizedTestCase):
	...
	def test_main(self):
		...
		try:
			...
			#	raise Error("vote_for_peer error")
			'''
			(result, response) = vote_for_peer(wallet_A_address, [node_B_puiblic_key], ["90000"])
			(result, response) = unvote_for_peer(wallet_A_address, [node_B_puiblic_key], ["10000"])
			
			
			while(True):
				if (getblockcount() / blocks_per_round - consensus_rounds == 1):
					(result, response) = vote_for_peer(wallet_A_address, [node_B_puiblic_key], ["10001"])
					break
				else:
					print (getblockcount())
					time.sleep(5)
			...
			(result, response) = withdraw_ont(wallet_A_address, [node_B_puiblic_key], ["10000"])
