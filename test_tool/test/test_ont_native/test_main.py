...
from test_ont_native.test_config import * 
...
class test_ont_native_1(ParametrizedTestCase):
	...
	def setUp(self):
		logger.open( "test_ont_native/"+self._testMethodName+".log",self._testMethodName)
	...
	def test_normal_024_transferFrom(self):
		try:
			(process, response) = approve1(test_config.contract_address,test_config.pay_address,Common.address_to_base58(test_config.contract_address), test_config.amount,test_config.node_index,0)#先approve
			if process:
				API.node().wait_gen_block()
				time.sleep(8)
			(process, response) = transferFrom(test_config.contract_address,Common.address_to_base58(test_config.contract_address),test_config.pay_address,Common.address_to_base58(test_config.contract_address), test_config.amount,test_config.sender2_node,test_config.sender2Type,errorcode=0)
			API.node().wait_gen_block()
			self.ASSERT(process, "")
		except Exception as e:
			logger.print (e.args[0]) 
