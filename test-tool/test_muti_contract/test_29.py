...
class TestMutiContract_29(ParametrizedTestCase):
    def test_main(self):
        logger.open("TestMutiContract_29.log", "TestMutiContract_29")
        result = False
        try:
            
            (contract_address, adminOntID, roleA_hex, roleB_hex, ontID_A, ontID_B, ontID_C) = set_premise("tasks/test_29.neo")
            ...
            (result, response) = delegate_user_role(contract_address, ontID_B, ontID_C, roleA_hex, "10", "1")
            if not result:
                raise("bind_user_role error")
